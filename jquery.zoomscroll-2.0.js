...
;(function($) {
	 ...
	 ************************/
	 var 	imgPath = '', // путь до изображения
	 		preloadedImg = new Image(), // инициируем изображение
    ...
    _mousewheel = function(e, delta) {
        ...
        /* VARIABLES
        ***********************************************/
        var imgW = $('#'+idImgView).width(); // ширина изображения
        var imgH = $('#'+idImgView).height(); // высота изображения
        
        var x = (e.layerX == undefined) ? e.offsetX : e.layerX ; // координата мыши x
        var y = (e.layerY == undefined) ? e.offsetY : e.layerY ; // координата мыши y
        ...
        ***********************************************/
        ...
        var pageX = Math.ceil( $('#'+idImgView).position().left - (x*((imgW2/imgW)-1)) );
        var pageY = Math.ceil( $('#'+idImgView).position().top - (y*((imgH2/imgH)-1)) );        
        ...
        ***********************************************/
        ...
        $('#'+idImgView).draggable( "destroy" );
                
        // изображение
        $('#'+idImgView).animate( { left: pageX, width: imgW2, top: pageY, height: imgH2 }, 600, _initDrag );            
    },
	...
	_myImageHandler = function() {
        ...
        $('#'+idImgOverlay+', #'+idImgClose).live('click', $.zoomscroll.close)
        ...
        $('#'+idImgView).live('mousedown', function(){return false;});
	...
	$.fn.zoomscroll = function() {
		...
		// обработчик: клик
		$(this).live("click", $.zoomscroll.open )
	...
	$.zoomscroll.open = function() {
        ...
        $('#'+idImgPreload).css({
            'left' : ($(this).find('img').width()/2 - 21),
            'top' : ($(this).find('img').height()/2 - 21)
        });
